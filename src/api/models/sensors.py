...
class SensorReadingResponse(BaseModel):
    ...
    sensor_type: str = Field(..., description="Tipo de sensor (TEMPERATURE, PRESSURE, FLOW)")
    ...
    is_warning: bool = Field(default=False, description="Indica si el valor está en rango de advertencia")
    is_critical: bool = Field(default=False, description="Indica si el valor está en rango crítico")
...
class SensorReadingsResponse(BaseModel):
    ...
    readings: List[SensorReadingResponse] = Field(..., description="Lista de lecturas de sensores")
...
class SensorDetailResponse(BaseModel):
    ...
    sensor_type: str = Field(..., description="Tipo de sensor (Temperatura, Presión, Caudal)")
    is_enabled: bool = Field(..., description="Si el sensor está habilitado")
    warning_threshold: Optional[float] = Field(None, description="Umbral de advertencia")
...
class UpdateSensorRequest(BaseModel):
    ...
    is_enabled: Optional[bool] = Field(None, description="Habilitar o deshabilitar el sensor")
    warning_threshold: Optional[float] = Field(None, description="Nuevo umbral de advertencia (null para limpiar)")
    critical_threshold: Optional[float] = Field(None, description="Nuevo umbral crítico (null para limpiar)")
