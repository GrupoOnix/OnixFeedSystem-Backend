...
async def verify():
    async with async_engine.begin() as conn:
        ...
        result = await conn.execute(
            text("SELECT EXISTS (SELECT FROM information_schema.tables WHERE table_name = 'slot_assignments')")
        )
        ...
        print(
            f"   {'✅ Migración completada' if with_line > 0 else '⚠️  Sin datos (normal si no había slot_assignments)'}\n"
        )
